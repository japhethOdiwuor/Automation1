...
WebUI.clearText(findTestObject('Merchandising/_My Account/_Users_EDIT- VIEW- NEW/input- Email'))

int n = rnd.nextInt(100) + 50

WebUI.setText(findTestObject('Merchandising/_My Account/_Users_EDIT- VIEW- NEW/input- Email'), (editEmail_Input+n+'.com'))

WebUI.clearText(findTestObject('Merchandising/_My Account/_Users_EDIT- VIEW- NEW/input- Max Number Logins'))
...
if (Message_AfterSaving.contains('Success')) {
    ...
    log.logWarning('PASSED: EMAIL Edited and Saved Successfully')
} else {
    log.logError('Displayed Message = ' + Message_AfterSaving)

    log.logError('ERROR: failed to Save Edited EMAIL Input')

    KeywordUtil.markError('ERROR while Saving edited EMAIL')
}

EMAIL_AfterEdit = WebUI.getAttribute(findTestObject('Merchandising/_My Account/_Users_EDIT- VIEW- NEW/input- Email'), 
    'value')
...
WebUI.setText(findTestObject('Merchandising/_My Account/input- Search'), EMAIL_AfterEdit)

WebUI.click(findTestObject('Merchandising/_My Account/btn- Search'))

WebUI.delay(2)

table_Result_EditedEMAIL = WebUI.getText(findTestObject('Merchandising/_My Account/table- Verify Data', [('row') : '1'
            , ('col') : '4']))

if (table_Result_EditedEMAIL == EMAIL_AfterEdit) {
    log.logWarning((('tableResult EMAIL after Search = ' + table_Result_EditedEMAIL) + ' | EMAIL set during Edit = ') + 
        EMAIL_AfterEdit)

    log.logWarning('Successfully EDITED EMAIL')
} else {
    log.logError((('tableResult EMAIL after Search = ' + table_Result_EditedEMAIL) + ' | EMAIL set during Edit = ') + 
        EMAIL_AfterEdit)

    log.logError('ERROR Encountered while EDITING EMAIL')
	KeywordUtil.markError("EMAIL Editing Failed")
}
